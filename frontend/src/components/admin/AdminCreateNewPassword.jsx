...
export default function AdminCreateNewPassword() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      let msg = err?.response?.data?.message || err?.response?.data?.error || 'Failed to reset password';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F7FAFC] p-6">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-md p-8 text-center">
          ...
          <button onClick={() => navigate('/admin/verify-reset-code')} className="bg-[#6B46C1] text-white rounded-xl py-2 px-4">Go to Verify</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F7FAFC] p-6">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-md p-8">
        <h1 className="text-2xl font-semibold text-gray-800 mb-2">Admin - Create New Password</h1>
