    ...
    const SOAPReportViewer = ({ caseId, onClose }) => {
      ...
      return (
        <div 
          className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 md:p-4 z-50 print-wrapper"
          onClick={(e) => {
        ...
        >
          <div className="bg-[#fffff2] rounded-[10px] w-full max-w-5xl my-4 md:my-8 relative max-h-[95vh] overflow-y-auto px-2 md:px-0">
            ...
            {/* === 1. TOP ACTION BAR (Download & Close) - Fixed Clickability === */}
            <div className="absolute right-4 top-4 flex items-center gap-4 z-50 no-print">
                <button 
                    onClick={(e) => {
                        e.stopPropagation(); // Prevents the background click logic from firing
                        handleDownload();
                    }}
                ...
                <button 
                    ...
                    className="text-3xl font-bold text-gray-600 hover:text-gray-800 cursor-pointer flex items-center leading-none h-8"
                    aria-label="Close"
                >×</button>
            </div>


            {/* === 2. VET NOTES (Preserved with your Truncation Logic) === */}
            {report.verification?.note && (
                <div className="px-2 md:px-8 mt-16"> {/* mt-16 ensures buttons don't cover the notes */}
            ...
            {/* === 3. HEADER SECTION (Logo, Stamps, Metadata) === */}
            <div className={`relative ${report.verification?.note ? 'pt-4' : 'pt-16'} px-8 flex flex-col md:flex-row items-center justify-between`}>
              {/* Logo Section */}
            ...
            {diagnoses.length > 0 && (
              <div className="px-2 md:px-8 mt-6 space-y-4">
                {diagnoses.map((d, i) => {
                  ...
                  return (
                    <div key={i} className="bg-[#f4f4f4] rounded-lg p-5 diagnoses-item">
